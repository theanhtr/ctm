...
{
    public class ExcelService<TEntity, TEntityDto, TExcelDto> : IExcelService<TExcelDto>
    {
        #region Fields
        protected readonly IBaseRepository<TEntity> _baseRepository;
        protected readonly IMapper _mapper;
        protected readonly IExcelWorker<TEntityDto, TExcelDto> _excelWorker;
        #endregion

        #region Constructor
        public ExcelService(IBaseRepository<TEntity> baseRepository, IMapper mapper, IExcelWorker<TEntityDto, TExcelDto> excelWorker)
        {
        ...
        public List<ExcelHeadersInfo> GetHeadersInfo(string filePath, string sheetContainsData, int headerRowIndex)
        {
            ...
            foreach (var header in headersInfo)
            {
                ...
                {
                    throw new ValidateException(StatusErrorCode.ExcelHeaderEmpty, ResourceVN.Excel_Header_Empty, null);
                }

                if (headerExists.Contains(header.HeaderName))
                {
                    var messageError = string.Format(ResourceVN.Excel_Header_Duplicate, header.HeaderName);
